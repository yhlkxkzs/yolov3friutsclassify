#!/usr/bin/env python3
"""YOLOv3-tiny 检测推理：权重为 YOLOv5 仓库导出的 .pt，经 torch.hub.load('ultralytics/yolov5', 'custom') 加载。"""
...
DEFAULT_WEIGHTS = REPO_ROOT / "models" / "yolov3_tiny_fruit_detect_best.pt"
...
def load_model(weights: Path, device: str):
    # 与训练时 YOLOv5 hub 栈一致；首次运行会从 GitHub 拉取 ultralytics/yolov5
    m = torch.hub.load(
        "ultralytics/yolov5",
        "custom",
        path=str(weights),
...
def main() -> int:
    p = argparse.ArgumentParser(description="YOLOv3-tiny（YOLOv5 hub）水果检测推理")
